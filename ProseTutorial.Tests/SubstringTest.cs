...
namespace ProseTutorial
{
    ...
    public class SubstringTest
    {
        ...
        [TestMethod]
        public void TestLearnCatString()
        {
            //parse grammar file
            Result<Grammar> grammar = CompileGrammar();
            //configure the prose engine
            SynthesisEngine prose = ConfigureSynthesis(grammar.Value);
            //create the example
            State input = State.CreateForExecution(
            grammar.Value.InputSymbol, "18");
            var examples = new Dictionary<State, object> { { input, "18181" } };
            var spec = new ExampleSpec(examples);
            //learn the set of programs that satisfy the spec
            ProgramSet learnedSet = prose.LearnGrammar(spec);
            //run the first synthesized program in the same input and check if
            //the output is correct
            IEnumerable<ProgramNode> programs = learnedSet.RealizedPrograms;
            //System.Console.WriteLine(programs.First());
            var output = programs.First().Invoke(input) as string;
            Assert.AreEqual("18181", output);
            State differentInput = State.CreateForExecution(
            grammar.Value.InputSymbol, "BC");
            output = programs.First().Invoke(differentInput) as string;
            Assert.AreEqual("BCBCB", output);
        }
        [TestMethod]
        public void TestLearnCatString2()
        {
            Result<Grammar> grammar = CompileGrammar();
            SynthesisEngine prose = ConfigureSynthesis(grammar.Value);
            State firstInput = State.CreateForExecution(
            grammar.Value.InputSymbol, "(Toby Miller)");
            State secondInput = State.CreateForExecution(
            grammar.Value.InputSymbol, "(Courtney Lynch)");
            var examples =
            new Dictionary<State, object> {
                { firstInput, "(Toby Miller)by Miller" },
                { secondInput, "(Courtney Lynch)urtney Lynch" } };
            var spec = new ExampleSpec(examples);
            ProgramSet learnedSet = prose.LearnGrammar(spec);
            IEnumerable<ProgramNode> programs = learnedSet.RealizedPrograms;
            //System.Console.WriteLine(programs.First());
            var output = programs.First().Invoke(firstInput) as string;
            Assert.AreEqual("(Toby Miller)by Miller", output);
            output = programs.First().Invoke(secondInput) as string;
            Assert.AreEqual("(Courtney Lynch)urtney Lynch", output);
            State differentInput = State.CreateForExecution(
            grammar.Value.InputSymbol, "(Alan Jasinska)");
            output = programs.First().Invoke(differentInput) as string;
            Assert.AreEqual("(Alan Jasinska)an Jasinska", output);
        }
